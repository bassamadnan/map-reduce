...
func (m *Master) mergeFiles() error {
	...
	for _, task := range m.Tasks {
		...
		if err != nil {
			fmt.Errorf("Error opening file for task %v-> %v", task.ID, task.Output)
		}
		defer file.Close()
		...
		for scanner.Scan() {
			...
			if err != nil {
				return fmt.Errorf("Error scanning file for task %v-> %v", task.ID, task.Output)
			}
			val, ok := kv.Value.(int) // make agnostic TODO
			if !ok {
				return fmt.Errorf("Error value expected int got %T", kv.Value)
